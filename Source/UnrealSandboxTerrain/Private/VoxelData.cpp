...
FORCEINLINE unsigned short TVoxelData::getRawMaterial(int x, int y, int z) const {
	auto index = x * voxel_num * voxel_num + y * voxel_num + z;
...
void serializeVoxelData(TVoxelData& vd, FBufferArchive& binaryData) {
	...
	if (vd.getDensityFillState() == TVoxelDataFillState::MIX) {
		...
		for (int x = 0; x < num; x++) {
			for (int y = 0; y < num; y++) {
				for (int z = 0; z < num; z++) {
					unsigned char density = vd.getRawDensity(x, y, z);
					binaryData << density;
				}
			}
		}
	...
	if (volume_state == 2) {
		for (int x = 0; x < num; x++) {
			for (int y = 0; y < num; y++) {
				for (int z = 0; z < num; z++) {
					unsigned short matId = vd.getRawMaterial(x, y, z);
					binaryData << matId;
				}
			}
		}
